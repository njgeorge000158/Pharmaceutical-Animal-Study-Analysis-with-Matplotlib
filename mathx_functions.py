...
#*******************************************************************************************
 #
 #  File Name:  mathx_functions.py
 #
 #  File Description:
 #      This Python script, mathx_functions.py, contains Python functions
 #      for math calculations. Here is the list:
 #
...
CONSTANT_LOCAL_FILE_NAME = 'mathx_functions.py'
...
#*******************************************************************************************
 ...
 #  series
 #          x_series
 #                          The parameter is the series holding the x values.
 #  series
 #          y_series
 #                          The parameter is the series holding the y values.
 #  integer
 #          degree_integer
 #                          This parameter is the degree of the polynomial.
 #
 #
 #  Date                Description                                 Programmer
 #  ---------------     ------------------------------------        ------------------
...
def return_regression_model_equation_coefficients \
    ...
    try:
        ...
        log_subroutines \
            .print_and_log_text \
                (f'The function, return_regression_model_equation_list, '
...
#*******************************************************************************************
 ...
 #
 #  Function Description:
 #      This function returns polynomial line as an np array.
 #
 #
...
def return_polynomial_line_series \
    ...
    try:
        sample_number_integer \
            = abs(int((x_series.max() - y_series.min()) / 2))
        ...
        log_subroutines \
            .print_and_log_text \
                (f'The function, return_polynomial_line_series, '
                 + f'in source file, {CONSTANT_LOCAL_FILE_NAME}, '
...
def return_equation_as_string \
    ...
    try:
        ...
        log_subroutines \
            .print_and_log_text \
                (f'The function, return_equation_as_string, '
                 + f'in source file, {CONSTANT_LOCAL_FILE_NAME}, '
...
def return_r_squared_value \
    ...
    try:
        ...
        pPoly1D = np.poly1d(coefficients_float_array)
    
        yhatList = pPoly1D(x_series)
    
        ybar_float = y_series.sum() / len(y_series)
    
        ssreg_float = ((yhatList-ybarFloatVariable)**2).sum()
    
        sstot_float = ((y_series - ybar_float)**2).sum()

        results_float = ssregFloatVariable / sstotFloatVariable
        ...
        log_subroutines \
            .print_and_log_text \
                (f'The function, return_r_squared_value, '
                 + f'in source file, {CONSTANT_LOCAL_FILE_NAME}, '
